...
**Fig. 1. Software interface in the Windows system and input/output format examples of GS-Impute 1.0 for general imputation. (A)** User interface of GS-Impute 1.0 for general imputation in the Windows system. **(B)** Example of the genotype file format used as input for general imputation, where "..." represents omitted nonessential information. The file may contain markers from multiple chromosomes. For a genotype files in CSV or TXT format, the column header immediately preceding data must be ref, alt, or QCcode. **(C)** Example of the reference panel format used as input for general imputation. The panel must be a VCF file corresponding to a single chromosome, and only markers on that chromosome can be imputed. Missing markers are not permitted in the reference panel. **(D)** Example of the output file format for general imputation.
...
**Fig. 3. Software interface in the Windows system and input/output format examples of GS-Impute 1.0 for reconstructive imputation. (A)** User interface of GS-Impute 1.0 for reconstructive imputation in the Windows system. **(B)** Example of the genotype file format used as input for reconstructive imputation, where "..." represents omitted nonessential information. The file may contain markers from multiple chromosomes. For a genotype files in CSV or TXT format, the column header immediately preceding data must be ref, alt, or QCcode. **(C)** Example of the position file format used as input for reconstructive imputation. The file may contain position information from multiple chromosomes. A genotype file containing chromosome and position information can also be used as the position file. **(D)** Example of the reference panel format used as input for reconstructive imputation. The panel must be a VCF file corresponding to a single chromosome, and only markers on that chromosome can be imputed. Missing markers are not permitted in the reference panel. **(E)** Example of the output file format for reconstructive imputation. Genotype values of 1 and 3 represent homozygous variants with two reference alleles and two alternate alleles, respectively. The genotype value of 2 represents a heterozygous variant containing one reference allele and one alternate allele.

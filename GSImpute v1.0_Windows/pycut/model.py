...
from pycut.func import getWindowMissing_index
...
def train_func(pro_signal,task_num,loop_num,epochs,epoch_num_all,epochNumTemp,window_size,num,TestAllMissing_index,trainyin_x,testyin_x,train_corrupt,batch_size,drop_perc,filter_size,stride,device,learning_rate,all_unit2,predictAll,end):
    for k in range(loop_num):
                ...
                for epoch in range(epochs):
                    ...
                    pro_signal.emit(proValue,nowEpochNum,epoch_num_all,'Task '+str(task_num)+' current progress: '+str(99.9)+'% ('+str(int(nowEpochNum))+'/'+str(int(epoch_num_all))+' epochs)',task_num)

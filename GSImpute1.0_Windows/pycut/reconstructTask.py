...
#Executing the task of reconstructive imputation
def Restructure(task_state_Signal,pro_signal,task_list_Signal,data1Name,data1Type,data2Name,dirPath,time,loop_num_all,window_size,task_num):
    stride=1
    ...
    task_list_Signal.emit('Task '+str(task_num)+': The number of samples for imputation is '+str(all_res_test_data[1]))
    task_list_Signal.emit('Task '+str(task_num)+': The number of loci for imputation is '+str(all_res_test_data[0]))
    task_list_Signal.emit('Task '+str(task_num)+': The missing rate of markers is '+ str(all_res_test_data[2])+'%')
...
#Getting the output file of reconstructive imputation
def RESgetCsv(data_A_path,data1Name,data1Type, data2Name,dirPath,saveName,loop_num):
    sampleName = []
    chrom = []
    vcfId = []
    REFList = []
    ...
    for loop in range(loop_num):
        ...
        dirPath1='/'.join(dirPath.split('/')[0:-1])
        str_csv = dirPath1+'/' +saveName
        if loop==0:
            df.to_csv(str_csv, index=False)
        else:
            df.to_csv(str_csv,mode='a',index=False,header=False)
    print("Reconstructive imputation finished")

#Getting the txt or csv output file of general imputation
def IMPgetTxtOrCsv(data_A_path,data1Name,data1Type,dirPath,saveName,chr,loop_num):
    headLine=[]
    isHead=1
...
#Getting the vcf output file of general imputation
def IMPgetVcf(dataName,dirPath,saveName,loop_num):
    for loop in range(loop_num):
